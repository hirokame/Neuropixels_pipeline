...
% DAmut a3370_a7832_a7789_a7793-241115-105745 % 3370
% data = 'Mouse-241205-103959'; % 6th Pav

% % DAmut a3373_a3371_a7836_a7838-241118-140421 % 3373 and 3371
% data = 'Mouse-241205-120638';


% load_type = '_UnivRAW_DeltaF_updown.mat'; % only use handles.Ch465 etc, so no need to use files below
load_type = '_UnivRAW_OffDmd.mat'; % only use handles.Ch465 etc, so no need to use files below
...
%% Transient triggered average
if draw_transient
    t = 200;
    threshold_low  = 3e-3;
    threshold_low  = 5.5e-3;
    % threshold_low  = 1e-2;
    threshold_high = 2.0;
    threshold_mid  = 1.0;
    window_size    = 1e3;
    transient_trigger(onlinedemod465,onlinedemod405,t,threshold_mid,window_size);title('onlinedemod465 - onlinedemod405')
    transient_trigger(onlinedemod465,onlinedemod560,t,threshold_mid,window_size);title('onlinedemod465 - onlinedemod560')
    transient_trigger(onlinedemod560,onlinedemod405,t,threshold_mid,window_size);title('onlinedemod560 - onlinedemod405')
    transient_trigger(onlinedemod560,onlinedemod465,t,threshold_mid,window_size);title('onlinedemod560 - onlinedemod465')
    transient_trigger(ydemod560_2,ydemod405_1,t,threshold_low,window_size);title('ydemod560_2 - ydemod405_1')
    transient_trigger(ydemod560_2,ydemod405_2,t,threshold_low,window_size);title('ydemod560_2 - ydemod405_2')
    transient_trigger(ydemod560_2,ydemod465_2,t,threshold_low,window_size);title('ydemod560_2 - ydemod465_2')
    transient_trigger(ydemod560_2,onlinedemod405,t,threshold_low,window_size);title('ydemod560_2 - onlinedemod405')
    transient_trigger(ydemod560_2,onlinedemod465,t,threshold_low,window_size);title('ydemod560_2 - onlinedemod465')
end
...
figure; set(gcf,'Position',[455   229   253   746])
subplot(3,1,1);plot(onlinedemod405./max(onlinedemod405),ydemod405_1./max(ydemod405_1));xlabel('normalized onlinedemod405'); ylabel('normalized ydemod405_1');AM_axis;axis equal
subplot(3,1,2);plot(onlinedemod465./max(onlinedemod465),ydemod465_1./max(ydemod465_1));xlabel('normalized onlinedemod465'); ylabel('normalized ydemod465_1');AM_axis;axis equal
subplot(3,1,3);plot(onlinedemod560./max(onlinedemod560),ydemod560_2./max(ydemod560_2));xlabel('normalized onlinedemod560'); ylabel('normalized ydemod560_2');AM_axis;axis equal

figure;
% subplot(1,1,1);plot(onlinedemod405./max(onlinedemod405),ydemod405_1./max(ydemod405_1));xlabel('normalized onlinedemod405'); ylabel('normalized ydemod405_1');AM_axis;axis equal;hold on;
% subplot(1,1,1);plot(onlinedemod405./max(onlinedemod405),ydemod405_2./max(ydemod405_2));xlabel('normalized onlinedemod405'); ylabel('normalized ydemod405_2');AM_axis;axis equal
subplot(1,1,1);plot(onlinedemod405./max(onlinedemod405),ydemod405_1./max(ydemod405_1));xlabel('normalized onlinedemod405'); ylabel('normalized offline');AM_axis;axis equal; hold on;
subplot(1,1,1);plot(onlinedemod405./max(onlinedemod405),ydemod405_2./max(ydemod405_2));xlabel('normalized onlinedemod405'); ylabel('normalized offline');AM_axis;axis equal
plot([0 1],[0 1],'k--')
legend('ydemod405_1','ydemod405_2','Location','best');
%% Parameters set
detrend_sec  = 10; % sec
...
for plot_signals_n = 1:5
    ...
    h_first = draw_step1(Ch405,Ch465,Ch560,detrend_sec,fs,TTs,portion,blowup_win_s,mag_factor,style);
    figure(h_first)
    switch plot_signals_n
        case 1
            title('Sensor 1')
...
for pair_nn = 1:size(pair_mat,1)
    ...
    figure;plot(dFF465);hold on;plot(dFF560);
    title([data ' dF/F for pair ' pair_mat_names(pair_nn)])
    xlabel('Time(ms)');ylabel('dF/F')
    legend('dFF465','dFF560')
end
%%
