function [ydemod, cfg, figs] = quickdemod_down_cutoff(y,fs,frqs,cutoff,down_to_fs,cfg_)
...
cfg.lowpass_cutoff__Hz  = cutoff; % change from 3Hz by Myles 241203
...
for ii = 1:size(D,2)
  D(:,ii) = filtfunc(D(:,ii));
    ...
    ix_middle1 = round(30*60*fs + [0 10]*fs);
    y1 = abs(D_orig(ix_middle1(1):ix_middle1(2))); % before filter
    y2 = abs(D(ix_middle1(1):ix_middle1(2))); % after filter
    ...
    plot(t_cut,y1_plot/std(y1_plot));hold on; % before filter
    plot(t_cut,y2_plot/std(y2_plot),'c');hold on; % after filter
    plot(t_cut,y1b_plot/std(y1b_plot),'m');hold on; % movmean before filter
    title('Demodulated signal before and after filter - for 10s from 30min')
    xlim(30*60 + [0 10]);ylabel('F');
    xlabel('Time(s)');legend(['raw demodulated signal ' channel_name],...
         [num2str(cutoff) 'Hz-low-pass filtered'],...
         'RAW 100ms-movmean')
