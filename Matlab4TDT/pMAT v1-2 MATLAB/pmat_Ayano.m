...
handles.Ch465F=[];handles.Ch405F=[];handles.Ch560H=[];
...
if handles.TDTFold~=0
    ...
    definput={'20000','2000'}; dlgtitle='Sample Buffer'; dims=[1 35];
    BufferInput=inputdlg(prompt,dlgtitle,dims,definput);
    if isempty(BufferInput)
        BufferInput={'20000','2000'}; end
    ...
    elseif isfield(handles.data.streams,'x465A')
        handles.Fs=handles.data.streams.x465A.fs;
        %Determine the timestamps for each value in the channel stream
        %based on the sampling frequency.
        Ts=((1:numel(handles.data.streams.x465A.data(1,:))) /handles.Fs);
        %Extract data from samples between the specified start and end times:
        EndTime=length(handles.data.streams.x465A.data)-EndBuffer;
        handles.Ts=Ts(StartBuffer:EndTime);
        handles.Ch465A=handles.data.streams.x465A.data(StartBuffer:EndTime);
    elseif isfield(handles.data.streams,'x465C')
        handles.Fs=handles.data.streams.x465C.fs;
        %Determine the timestamps for each value in the channel stream
        %based on the sampling frequency.
        Ts=((1:numel(handles.data.streams.x465C.data(1,:))) /handles.Fs);
        %Extract data from samples between the specified start and end times:
        EndTime=length(handles.data.streams.x465C.data)-EndBuffer;
        handles.Ts=Ts(StartBuffer:EndTime);
        handles.Ch465C=handles.data.streams.x465C.data(StartBuffer:EndTime);
    elseif isfield(handles.data.streams,'x465E')
        handles.Fs=handles.data.streams.x465E.fs;
        %Determine the timestamps for each value in the channel stream
        %based on the sampling frequency.
        Ts=((1:numel(handles.data.streams.x465E.data(1,:))) /handles.Fs);
        %Extract data from samples between the specified start and end times:
        EndTime=length(handles.data.streams.x465E.data)-EndBuffer;
        handles.Ts=Ts(StartBuffer:EndTime);
        handles.Ch465E=handles.data.streams.x465E.data(StartBuffer:EndTime);
    elseif isfield(handles.data.streams,'x465G')
        handles.Fs=handles.data.streams.x465G.fs;
        %Determine the timestamps for each value in the channel stream
        %based on the sampling frequency.
        Ts=((1:numel(handles.data.streams.x465G.data(1,:))) /handles.Fs);
        %Extract data from samples between the specified start and end times:
        EndTime=length(handles.data.streams.x465G.data)-EndBuffer;
        handles.Ts=Ts(StartBuffer:EndTime);
        handles.Ch465G=handles.data.streams.x465G.data(StartBuffer:EndTime);
    elseif isfield(handles.data.streams,'x560B')
        handles.Fs=handles.data.streams.x560B.fs;
        %Determine the timestamps for each value in the channel stream
        %based on the sampling frequency.
        Ts=((1:numel(handles.data.streams.x560B.data(1,:))) /handles.Fs);
        %Extract data from samples between the specified start and end times:
        EndTime=length(handles.data.streams.x560B.data)-EndBuffer;
        handles.Ts=Ts(StartBuffer:EndTime);
        handles.Ch560B=handles.data.streams.x560B.data(StartBuffer:EndTime);
    elseif isfield(handles.data.streams,'x560D')
        handles.Fs=handles.data.streams.x560D.fs;
        %Determine the timestamps for each value in the channel stream
        %based on the sampling frequency.
        Ts=((1:numel(handles.data.streams.x560D.data(1,:))) /handles.Fs);
        %Extract data from samples between the specified start and end times:
        EndTime=length(handles.data.streams.x560D.data)-EndBuffer;
        handles.Ts=Ts(StartBuffer:EndTime);
        handles.Ch560D=handles.data.streams.x560D.data(StartBuffer:EndTime);
    elseif isfield(handles.data.streams,'x560F')
        handles.Fs=handles.data.streams.x560F.fs;
        %Determine the timestamps for each value in the channel stream
        %based on the sampling frequency.
        Ts=((1:numel(handles.data.streams.x560F.data(1,:))) /handles.Fs);
        %Extract data from samples between the specified start and end times:
        EndTime=length(handles.data.streams.x560F.data)-EndBuffer;
        handles.Ts=Ts(StartBuffer:EndTime);
        handles.Ch560F=handles.data.streams.x560F.data(StartBuffer:EndTime);
    elseif isfield(handles.data.streams,'x560H')
        handles.Fs=handles.data.streams.x560H.fs;
        %Determine the timestamps for each value in the channel stream
        %based on the sampling frequency.
        Ts=((1:numel(handles.data.streams.x560H.data(1,:))) /handles.Fs);
        %Extract data from samples between the specified start and end times:
        EndTime=length(handles.data.streams.x560H.data)-EndBuffer;
        handles.Ts=Ts(StartBuffer:EndTime);
        handles.Ch560H=handles.data.streams.x560H.data(StartBuffer:EndTime);
    else
    ...
    elseif isfield(handles.data.streams,'x405A')
        %This is repeated if a control stream is not found with the
        %expected name.
        Ch405A=handles.data.streams.x405A.data;
        handles.Ch405A=Ch405A(StartBuffer:EndTime);
    elseif isfield(handles.data.streams,'x405C')
        %This is repeated if a control stream is not found with the
        %expected name.
        Ch405C=handles.data.streams.x405C.data;
        handles.Ch405C=Ch405C(StartBuffer:EndTime);
    elseif isfield(handles.data.streams,'x405E')
        %This is repeated if a control stream is not found with the
        %expected name.
        Ch405E=handles.data.streams.x405E.data;
        handles.Ch405E=Ch405E(StartBuffer:EndTime);
    elseif isfield(handles.data.streams,'x405G')
        %This is repeated if a control stream is not found with the
        %expected name.
        Ch405G=handles.data.streams.x405G.data;
        handles.Ch405G=Ch405G(StartBuffer:EndTime);
    else
...
if 1 %If "Ca2+ Data" is selected:
    if size(handles.Ts,1)<size(handles.Ts,2)
%     tmp=[handles.Ts', handles.Ch490', handles.Ch405'];
    tmp=[handles.Ts', handles.Ch405A', handles.Ch465A', handles.Ch560B', ...
                      handles.Ch405C', handles.Ch465C', handles.Ch560D', ...
                      handles.Ch405E', handles.Ch465E', handles.Ch560F', ...
                      handles.Ch405G', handles.Ch465G', handles.Ch560H' ];
    else
%     tmp=[handles.Ts, handles.Ch490, handles.Ch405];
    tmp=[handles.Ts,  handles.Ch405A, handles.Ch465A, handles.Ch560B, ...
                      handles.Ch405C, handles.Ch465C, handles.Ch560D,...
...
%     header=["TimeStamp","Signal","Control"];
    header=["TimeStamp","Ch405A","Ch465A","Ch560B",...
                        "Ch405C","Ch465C","Ch560D",...
                        "Ch405E","Ch465E","Ch560F",...
                        "Ch405G","Ch465G","Ch560H"];
...
if 1 %If "All Event Data" is selected:
