...
disp('running ExtractKilosortData')
...
for subsesid = 1:length(KiloSortPaths)
    ...
    disp('ISEMPTY?')
    disp(isempty(myClusFile))
    disp(fullfile(KiloSortPaths{subsesid}))
    ...
    if isempty(myClusFile) || Params.RunQualityMetrics
        ...
        if isempty(myClusFile)
            ...
            disp('no cluster_group.tsv')
        else
            disp(myClusFile(1).folder)
            disp(myClusFile(1).name)
    ...
    if Params.RunQualityMetrics
        ...
        for id = 1:length(rawD)
            ...
            if length(rawD) > 1 % DO NOT DELETE!
                ...
                idx = sp.SessionID == 1;
                disp(['Processing session ' num2str(id) ' of ' num2str(length(rawD))])
                disp('savePath')
                disp(savePath)
            end
            ...
            disp(['Quality metrics exist: ' num2str(qMetricsExist)])
            ...
            if ~qMetricsExist || Params.RedoQM % if quality metrics do not exist or we want to redo them
                ...
                disp('Loading existing quality metrics from parquet file');
                disp(d)
                ...
                % !!! unitType = bc.qm.getQualityUnitType(paramBC, qMetric, qMetricsPath);
                tsvFile = fullfile(qMetricsPath, 'templates._bc_unit_labels.tsv');
                % print tsvFile PATH
                disp('TSV file path:');
                disp(tsvFile);
                data = readtable(tsvFile, 'FileType', 'text', 'Delimiter', '\t', 'ReadVariableNames', false);
                unitType = data.Var1;
                %                 unitType(:) = 1; ???


                % Commmented by CB for now
                npyFile = fullfile(kilosortPath, 'spike_clusters.npy');
                disp(['reading', npyFile]);

                % if ~exist(npyFile, 'file')
                %     error('file non exist: %s', npyFile);
                % else
                %     disp('file exist');
                % end
                % spike_templates_0idx = readNPY([kilosortPath filesep 'spike_clusters.npy']); % changed back 20230920 JF
                spike_templates_0idx = readNPY(npyFile); % changed back 20230920 JF
                ...
                try
                    ...
                    if isempty(tmpfile)
                        disp('File not found: templates.qualityMetricDetailsforGUI.mat');
                    end
            ...
            %% Apply QM findings:
            disp('Only use units good and mua')
            ...
            Good_IDtmp = nan(1, length(Good_IDtmp)); 

            % label good as 1
            Good_IDtmp(qMetricclusterID(unitType == 1)) = 1; 

            % label mua as 1
            Good_IDtmp(qMetricclusterID(unitType == 2)) = 1; 

            % else
            Good_IDtmp(isnan(Good_IDtmp)) = 0; 
