function res = summaryFunctionalPlots(UMFiles, whichMetric, groupVector, UseKSLabels, pltDayPairFig)
    ...
    for midx = 1:length(UMFiles)
        ...
        % 修改正则表达式来匹配mmddyyyy格式的日期
        % 尝试匹配路径中的\mmddyyyy\格式
        days{midx} = cellfun(@(y) datenum(y, 'mmddyyyy'), ...
            cellfun(@(x) regexp(x.folder, '\\\d{8}\\', 'match', 'once'), ...
            UMparam.RawDataPaths, 'uni', 0), 'uni', 0);
        
        % 如果没有找到，尝试匹配路径末尾的\mmddyyyy格式
        if all(cell2mat(cellfun(@(x) isempty(x), days{midx}, 'uni', 0)))
            days{midx} = cellfun(@(y) datenum(y, 'mmddyyyy'), ...
                cellfun(@(x) regexp(x.folder, '\\\d{8}$', 'match', 'once'), ...
                UMparam.RawDataPaths, 'uni', 0), 'uni', 0);
        ...
        % 如果没有找到，尝试匹配Unix风格的路径/mmddyyyy/格式
        if all(cell2mat(cellfun(@(x) isempty(x), days{midx}, 'uni', 0)))
            days{midx} = cellfun(@(y) datenum(y, 'mmddyyyy'), ...
                cellfun(@(x) regexp(x.folder, '/\d{8}/', 'match', 'once'), ...
                UMparam.RawDataPaths, 'uni', 0), 'uni', 0);
        ...
        % 如果没有找到，尝试匹配Unix风格路径末尾的/mmddyyyy格式
        if all(cell2mat(cellfun(@(x) isempty(x), days{midx}, 'uni', 0)))
            days{midx} = cellfun(@(y) datenum(y, 'mmddyyyy'), ...
                cellfun(@(x) regexp(x.folder, '/\d{8}$', 'match', 'once'), ...
                UMparam.RawDataPaths, 'uni', 0), 'uni', 0);
