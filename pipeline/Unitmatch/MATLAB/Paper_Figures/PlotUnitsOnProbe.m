...
try
    if isunix
       % Adjusted regexp pattern for Unix-style paths with forward slashes
        ...
        % days = cellfun(@(y) datenum(y), cellfun(@(x) regexp(x.folder,'\','match'), UMparam.RawDataPaths, 'uni', 0), 'uni', 0);
        dateMatches = cellfun(@(x) regexp(x.folder, '\d{8}', 'match'), UMparam.RawDataPaths, 'uni', 0);

        % 将日期字符串转换为 datenum（需指定输入格式为 'mmddyyyy'）
        days = cellfun(@(y) datenum(y{1}, 'mmddyyyy'), dateMatches, 'uni', 0);
    end
