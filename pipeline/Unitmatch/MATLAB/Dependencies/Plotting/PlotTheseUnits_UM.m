...
for uid = 1:nclus
    ...
    %Load channels
    ChanIdx = find(cell2mat(arrayfun(@(Y) norm(channelpos(WaveformInfo.MaxChannel(uid,1),:)-channelpos(Y,:)),1:size(channelpos,1),'UniformOutput',0))<150); %Averaging over 10 channels helps with drift
    Locs = channelpos(ChanIdx,:);
    % disp(['uid 1, ', mat2str(uid)])
    % disp(['Locs size 1 : ', mat2str(size(Locs))]);
...
for pairid=1:length(Pairs)
    ...
    for uidx = 1:length(Pairs{pairid})
        ...
        %Load channels
        % CHANGE 75 INTO 150
        ChanIdx = find(cell2mat(arrayfun(@(Y) norm(channelpos(WaveformInfo.MaxChannel(uid,cv),:)-channelpos(Y,:)),1:size(channelpos,1),'UniformOutput',0))<150); %Averaging over 10 channels helps with drift
        Locs = channelpos(ChanIdx,:);
        % disp(mat2str(norm(channelpos(WaveformInfo.MaxChannel(uid,cv),:)-channelpos(5,:))));
        % disp(["uid,", mat2str(uid), ' cv, ', mat2str(cv)]);
        % disp(['max channel: ', mat2str(WaveformInfo.MaxChannel(uid,cv))]);
        % disp(['channel pos: ', mat2str(channelpos(WaveformInfo.MaxChannel(uid,cv),:))]);
        % disp(['Locs size 2 : ', mat2str(size(Locs))]);

        if isempty(Locs)
            disp('Locs is empty, skipping this iteration');
            flag = 1;
        ...
        Locs = Locs(:,sortdims);
        % disp(['Locs size sort: ', mat2str(size(Locs))]);
...
%         scatter(squeeze(ProjectedLocationPerTPAllFlips(sortdims(1),uid,takesamples,cv,flipidx)),squeeze(ProjectedLocationPerTPAllFlips(sortdims(2),uid,takesamples,cv,flipidx)),30,takesamples,'filled')
        ...
        hold on
        %
        %disp(['Locs size: ', mat2str(size(Locs))]);
        % disp(['addforspace size: ', mat2str(size(addforspace))]);
        %assert(isequal(size(Locs(:,1)), size(addforspace)), '尺寸不匹配！');
