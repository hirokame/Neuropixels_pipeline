...
for uid = 1:nclus
    ...
    if length(tmppath)>1
        % Probably stitched:
        % disp("length(tmppath)>1")
        % disp(GoodRecSesID(uid));
    ...
    % disp(Path4UnitNPY{uid});
    % disp(exist(Path4UnitNPY{uid}));

    if ~exist(Path4UnitNPY{uid})
        disp(Path4UnitNPY{uid})
    end
    if exist(Path4UnitNPY{uid}) && ~RedoExtraction
        ...
        if ~(GoodRecSesID(uid) == Currentlyloaded) % Only load new memmap if not already loaded
            ...
            if ispc
                ...
                disp('Memory usage:')
                disp(spikeFile.bytes)
                disp('Max possible array bytes:')
                disp(userview.MaxPossibleArrayBytes)
        ...
        % only use the idx1 that are within the behavioral session
        ...
        % disp('spikeIndicestmp')
        % disp(spikeIndicestmp(1))
